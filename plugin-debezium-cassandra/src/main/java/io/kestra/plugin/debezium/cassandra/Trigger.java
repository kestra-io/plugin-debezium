...
@NoArgsConstructor
@Schema(
    title = "Consume messages periodically from a Cassandra database via change data capture and create one execution per batch.",
    description = "If you would like to consume each message from change data capture in real-time and create one execution per message, you can use the [io.kestra.plugin.debezium.cassandra.RealtimeTrigger](https://kestra.io/plugins/plugin-debezium/triggers/io.kestra.plugin.debezium.cassandra.realtimetrigger) instead."
)
@Plugin(
    examples = {
        @Example(
...
public class Trigger extends AbstractDebeziumTrigger implements CassandraInterface, AbstractDebeziumInterface {
    @Builder.Default
